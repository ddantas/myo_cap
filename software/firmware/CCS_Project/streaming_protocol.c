...
void left_shift_sample_as_uint128(uint16_t sample, uint8_t shift, uint8_t bits_per_Sample_,uint64_t* shifted_sample){
    ...
    shifted_sample[0] = shifted_sample[0] << (sizeof(uint64_t) - bits_per_Sample_);                     // Put the Sample as much left as possible.
    shifted_sample[0] = shifted_sample[0] >> (2 * sizeof(uint64_t) - bits_per_Sample_ - shift);         // Shift Right Reverse with bits_per_Sample_ of offset
