...
void timer0AInterrupt(void)
{
...

    {
        ...
        acquired_sample = function_gen(tiva_actual_state.wave_form, tiva_actual_state.timestamp, tiva_actual_state.func_gen_frequence); // Generates e Store a Sample of the Function Generator with the Wave Form Chosen.
...
void configurations()
{
    ...
    SysCtlClockSet(SYSCTL_SYSDIV_5 | SYSCTL_USE_PLL | SYSCTL_XTAL_16MHZ | SYSCTL_OSC_MAIN);                            // Sets the Tiva frequence as 40 MHz.
